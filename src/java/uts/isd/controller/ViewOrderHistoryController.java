...
public class ViewOrderHistoryController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ViewOrderHistoryController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ViewOrderHistoryController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ArrayList<Order> orders = manager.listOrderByCustomer(customer.getCustomerId(), orderIdSearch, dateSearch);
            System.out.println("View ok");
            request.setAttribute("orders", orders);
            request.getRequestDispatcher("viewListOrdersHistories.jsp").forward(request, response);
        } catch (Exception ex) {
